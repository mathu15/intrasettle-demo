...
const WBOOrderBook = ({ data, setData }) => {
  ...
  useEffect(() => {
    ...
    let transactionsURL = "https://thebsv.tech/exchange/getordertransactions/CAC-SUB901-0001";
  ...
  const DisplayOne = () => {
    if (activeone === 0) {
      return <WBOmarketDepth buy={pastData} sell={sellData} />;
    } else if (activeone === 1) {
      return <WBOVolume buy={pastData} sell={sellData} />;
    }
  };
  const DisplayTwo = () => {
    if (activetwo === 0) {
      return <WBOMatchHistory buy={pastData} sell={sellData} />;
    }
  };
  const DisplayThree = () => {
    if (activethree === 0) {
  ...
  const DisplayFour = () => {
    if (activefour === 0) {
      ...
      return <WBOCompleted data={allBuyData} />;
    }
  };

  const DisplayFive = () => {
    if (activefive === 0) {
      return <WBOBuyOrder price={price} />;
    } else if (activefive === 1) {
      return <WBOSellOrder price={price} />;
    }
  };
